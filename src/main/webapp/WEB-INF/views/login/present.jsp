...
<script type="text/javascript">
    ...
    function bannerShow() {
 	...
 	window.onload = function() {
	  // 로그인 안되었을때
	  if('false' == 'false'){
		var chkCid = "1282442854.1719998667";
...
/*   require(['TweenMax'], function(a){
	...
	$(function(){
		...
		function initGiftShopSlides() {

			if ($giftsStepSlide || $giftsWrapSlide) {
				$giftsStepSlide.destroy(true, true);
				$giftsWrapSlide.destroy(true, true);
			}
			...
			$giftsStepSlide = new Swiper('.slide_gifts_step', {
				loop : true,
				speed : 300,
				centeredSlide : true,
				slidesPerView : 1,
				roundLengths : true,
				navigation : {
					prevEl: '.gifts_step .buttonPrev',
					nextEl: '.gifts_step .buttonNext',
				},
				autoplay : {
					delay : 3500,
			...
			$giftsWrapSlide = new Swiper('.slide_gifts_wrap', {
				loop : true,
				speed : 300,
				centeredSlide : true,
				...
				roundLengths : true,
				pagination : {
				el : '.gifts_wrap .swiper-pagination',
					clickable : true,
				},
				autoplay : {
					delay : 3500,
		...
		$(".sale_percent").each(function(){
			let text = $(this).text();
			text = text.replace("0000", "만");
			text = text.replace("000", "천");
			$(this).text(text);
		});

		$('.gifts_sec.gifts_typeC').each(function() {
		    var len = $(this).children().find('li').length;
		    var button = $(this).children().find('.btn_A'); 
		    if (len > 9) {
		        var nonDiv = $('<div/>').css('display','none');
		        var li = $(this).children().find('li');
		        li.eq(8).append(nonDiv);
		        for (var i = 9; i < len; i++) {
		            nonDiv.append(li.eq(i));
		        }
		        
		        button.click(function() {
		            nonDiv.children('li').each(function() {
		                $(this).parents('ul').append($(this));
		            });
		...
		$('.gifts_sec.gifts_typeA').each(function() {
		    var len = $(this).children().find('li').length;
		    var button = $(this).children().find('.btn_A'); 
		    if (len > 10) {
		        var nonDiv = $('<div/>').css('display','none');
		        var li = $(this).children().find('li');
		        li.eq(9).append(nonDiv);
 	...
 	function goPresent(ids, amount){
	       ...
	       var currentPath = "/kr/ko/shop/item/gift";
